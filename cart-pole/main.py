...
try:
    ...
    for t in range(100000):
        ...
        for step in itertools.count():
            steps_all += 1
            epsilon = decay(epsilon_start, epsilon_end, epsilon_decay_rate, steps_all)
            ...
            if steps_all % batch_size == 0:
                # experiences = random.sample(replayMemory[-1000:], batch_size)
                experiences = random.sample(replayMemory, batch_size)
                # experiences = replayMemory[-batch_size:]
                # experiences = replayMemory[-10:]
            ...
            # stop learning process when 5 out of last 10 episodes reach 1000 steps
            if sum(item > 1000 for item in perf[-10:]) >= 5:
                ...
                while True:
                    ...
                    if done or exec_step > 10000:
                        print(f"Steps {exec_step}")
